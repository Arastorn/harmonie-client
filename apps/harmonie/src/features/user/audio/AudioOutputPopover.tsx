...
export const AudioOutputPopover = ({ anchorRef, onClose }: AudioOutputPopoverProps) => {
  ...
  const getLabel = (device: { deviceId: string; label: string }) => {
    if (device.deviceId === 'default') return t('audio.output.default');
    return device.label || t('audio.output.unknown');
  };
  ...
  const content = (
    ...
    >
      ...
      {devices.length === 0 && (
        <p className="text-xs text-text-3 px-2 py-1">{t('audio.output.noDevices')}</p>
      )}
      ...
      {devices.map((device) => {
        ...
        return (
          ...
          >
            <Volume2 size={13} className="shrink-0 text-text-3" />
      ...
      {needsPermission && (
        ...
        >
          ...
          <span>{requesting ? t('audio.output.requesting') : t('audio.output.grantAccess')}</span>
        </button>
      )}
    </div>
  );
